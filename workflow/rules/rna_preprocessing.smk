...
rule fastp_qc_trim_rna:
    input:
        ...
        r2 = get_rna_fq2
    output:
        r1 = temp("rna/trimmed/{sample}_{sampletype}_R1_trimmed.fastq.gz"), # 标记为temp，如果下游只用BAM，它会自动清理
        r2 = temp("rna/trimmed/{sample}_{sampletype}_R2_trimmed.fastq.gz"),
        json = "rna/trimmed/{sample}_{sampletype}_rna_fastp.json",
        html = "rna/trimmed/{sample}_{sampletype}_rna_fastp.html"
    log:
...
rule mark_duplicates_rna:
    ...
    output:
        bam = "rna/align/{sample}_{sampletype}_dedup.bam", # 中间文件
